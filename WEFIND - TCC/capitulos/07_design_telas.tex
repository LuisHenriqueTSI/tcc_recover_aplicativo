...
A tela de login reúne os campos de e-mail e senha, a recuperação de credenciais e a possibilidade de conhecer o aplicativo antes do cadastro. Essa organização reduz o número de decisões iniciais e orienta o usuário para as ações essenciais.
...
A localização é um recurso central do sistema, pois permite relacionar publicações, avistamentos e distâncias. A Figura~\ref{fig:tela_mapa} reúne a solicitação de localização, o mapa interativo e a consulta de rota.
...
A interface cartográfica solicita a permissão de GPS, apresenta marcadores associados às publicações e permite consultar uma rota até o ponto selecionado. Esses elementos apoiam a busca sem substituir a avaliação do usuário sobre as condições do local.

O formulário unificado de registro contempla as situações mais frequentes da plataforma: o tutor que perdeu o animal, a pessoa que encontrou um animal e o cidadão que apenas realizou um avistamento. A Figura~\ref{fig:tela_registro_perdi_encontrei} apresenta esses caminhos principais.
...
A escolha da modalidade define os campos exibidos no formulário. O registro de perda prioriza a identificação e as características do animal; o registro de encontro permite informar que o animal foi localizado; e o avistamento associa a observação a uma posição geográfica. A separação dos caminhos reduz ambiguidades no envio das informações.

A ficha de cada publicação concentra as informações necessárias para consulta e acompanhamento do caso, conforme a Figura~\ref{fig:tela_detalhes}.
...
A visualização pública exibe fotografias, características físicas, localização aproximada e meios de envio de pistas. Quando a publicação pertence ao usuário, a mesma área oferece ações de edição, renovação e encerramento do caso, mantendo distintos os níveis de acesso.

O sistema também cruza atributos de publicações para apresentar correspondências potenciais, como mostra a Figura~\ref{fig:tela_correspondencias}.
...
A área de matching organiza candidatos por similaridade e distância, permitindo comparar características antes do contato entre os participantes. O resultado é apresentado como apoio à decisão, sem substituir a confirmação humana da identidade do animal.

Por fim, os animais cadastrados pelo usuário podem ser consultados na carteirinha digital, apresentada na Figura~\ref{fig:tela_meus_pets_rg}.
...
A carteirinha digital reúne os dados essenciais do animal e um código QR para facilitar sua identificação. O painel de pets também permite acessar esses dados e acionar o fluxo de alerta quando necessário, fortalecendo a prevenção sem duplicar o cadastro de uma publicação.
