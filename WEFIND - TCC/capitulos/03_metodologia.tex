...
Essa organização permitiu realizar verificações incrementais em aparelhos celulares reais ao final de cada ciclo, contribuindo para a identificação antecipada de falhas. A validação sistemática da cobertura dos requisitos e da estabilidade da aplicação permanece prevista para a etapa de testes descrita no Capítulo 8.
...
O fluxo produtivo estruturou-se em cinco colunas sequenciais no quadro visual. Inicialmente, a coluna de Repositório Geral de Tarefas reuniu os 46 requisitos funcionais, os 14 requisitos não funcionais e as oportunidades de aprimoramento mapeadas a partir da pesquisa de campo e da análise comparativa. A partir desse repositório, as tarefas priorizadas para o ciclo imediato transitavam para a coluna A Fazer, de onde eram puxadas individualmente para a coluna Em Andamento à medida que o código-fonte ou artefato de modelagem entrava em fase ativa de confecção. Uma vez implementada, a funcionalidade avançava para a coluna de Testes e Revisão, na qual recebia inspeção visual, verificações preliminares de regras de negócio em dispositivo físico e conferência de contraste e usabilidade. A aprovação formal dos critérios de aceitação e a confirmação da cobertura dos requisitos permanecem vinculadas à etapa de testes planejada.
...
A união entre a pesquisa de campo, a governança visual pelo Kanban e os princípios de engenharia de software orientou a construção de uma aplicação móvel alinhada às necessidades identificadas no grupo consultado. O Capítulo 4 apresenta a especificação dos requisitos e os modelos conceituais da plataforma proposta.
