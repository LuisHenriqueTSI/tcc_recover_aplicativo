...
Na sequência da investigação, mapeou-se o comportamento prático dos participantes durante episódios reais de desaparecimento, identificando os canais analógicos e digitais mais utilizados para a busca (Figura~\ref{fig:pesquisa_meios}).
...
Para compreender a vivência prática da comunidade frente à problemática dos desaparecimentos, consultou-se os voluntários sobre experiências prévias de perda ou localização de animais desorientados nas ruas (Figura~\ref{fig:experiencia_perda}).

\begin{figure}[H]
    \centering
    \caption{Experiência prévia dos participantes com perda ou resgate de animais domésticos}
    \label{fig:experiencia_perda}
    \includegraphics[width=0.75\textwidth]{imagens/01_pesquisa/figura02_experiencia_perda.png}
    \fonte{Dados da pesquisa (Google Forms, 2026)}
\end{figure}
...
O segundo incremento contemplou o núcleo espacial e o georreferenciamento, mediante a integração do subsistema GPS pelo pacote \textit{expo-location}, a renderização do mapa interativo com \textit{react-native-maps}, a plotagem de marcadores dinâmicos diferenciados por espécie e a ordenação do mural de publicações conforme a proximidade física do usuário. No terceiro incremento, desenvolveu-se a gestão de publicações e mídias, contemplando os fluxos completos de cadastro de anúncios de animais nas categorias de perdidos, encontrados e adoção, o acoplamento da câmera e galeria por meio do pacote \textit{expo-image-picker}, a rotina de compressão fotográfica local com o \textit{expo-image-manipulator} e o armazenamento seguro de arquivos no Supabase Storage.
