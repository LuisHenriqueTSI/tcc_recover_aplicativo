...
\ifversaoorientadora
Para conferir rigor acadêmico e assegurar a cobertura integral do escopo do projeto, cada caso de teste foi desenhado e especificado para validar um conjunto articulado de Requisitos Funcionais (RF) estabelecidos no Capítulo 4. A suíte contempla 25 casos de teste estruturados (CT01 a CT25), combinando fluxos de caminho feliz (execução regular das rotinas), validações de robustez frente a dados inconsistentes e operações críticas em banco de dados e sensores nativos. Dessa forma, estrutura-se a cobertura planejada de 100\% dos 46 requisitos funcionais do aplicativo, cuja rastreabilidade individual completa frente às necessidades diagnosticadas na pesquisa de campo encontra-se catalogada no Apêndice D.

O Quadro~\ref{quadro:testes} consolida a especificação detalhada dos 25 cenários de testes funcionais estruturados para o sistema.
\else
Para conferir rigor acadêmico e assegurar a cobertura integral do escopo do projeto, cada caso de teste foi desenhado para validar um conjunto articulado de Requisitos Funcionais (RF) estabelecidos no Capítulo 4. A suíte contempla 25 casos de teste estruturados (CT01 a CT25), combinando fluxos de caminho feliz (execução regular das rotinas), validações de robustez frente a dados inconsistentes e operações críticas em banco de dados e sensores nativos. Dessa forma, atesta-se a cobertura de 100\% dos 46 requisitos funcionais do aplicativo, cuja rastreabilidade individual completa frente às necessidades diagnosticadas na pesquisa de campo encontra-se catalogada no Apêndice D.

O Quadro~\ref{quadro:testes} consolida a matriz integral dos 25 casos de testes funcionais executados sobre a versão estável da aplicação.
\fi
...
\hline
\multicolumn{6}{|c|}{\textbf{Quadro 6: Matriz de Testes Funcionais do Sistema WeFIND}} \\ \hline
...
\hline
\multicolumn{6}{|c|}{\textbf{Quadro 6: Matriz de Testes Funcionais do Sistema WeFIND (Continuação)}} \\ \hline
\textbf{ID} & \textbf{Requisitos} & \textbf{Cenário de Teste} & \textbf{Procedimento Executado} & \textbf{Resultado Esperado} & \textbf{Situação} \\ \hline
\endhead
...
\textbf{CT16} & RF22, RF23, RF24, RF25, RF26 & Ciclo Comunitário de Avistamentos e Atualização de Localização & Registro de avistamento com foto e pino, confirmação de correspondência e consulta do histórico & Criação do relato, atualização da localização mais recente da publicação, notificação ao autor e preservação da linha do tempo & \statusTeste \\ \hline
...
\textbf{CT23} & RF44 & Apuração e Exibição do Ranking Semanal & Registro de ações com pontuação e consulta do ranking municipal na tela de perfil & Cálculo em tempo real dos pontos, ordenação do pódio e exibição da colocação do usuário & \statusTeste \\ \hline
\textbf{CT24} & RF45 & Cadastro e Busca de Lares Temporários & Ativação de perfil de acolhedor com critérios de moradia e envio de solicitação de lar & Persistência segura no banco, exibição com selo verificado e envio de pedido direto & \statusTeste \\ \hline
\textbf{CT25} & RF46 & Correspondência Multicritério e Controle de Falso Positivo & Cadastrar um animal encontrado ou avistamento com espécie, raça, cor, sexo, porte e localização compatíveis com publicação não encerrada; repetir o cenário com atributos incompatíveis e selecionar ``Não, é outro animal'' & Priorização das publicações compatíveis por pontuação e distância; exclusão de registros do próprio usuário; confirmação obrigatória antes da atualização; atualização da localização e registro do avistamento somente no aceite; criação de nova publicação ou descarte da sugestão no cenário negativo & \statusTeste \\ \hline
\end{longtable}
}
...
A integração equilibrada desses três eixos (os vinte e cinco casos de teste funcionais, o planejamento de usabilidade com a escala SUS e os ensaios de desempenho em campo) assegura uma base sólida para certificar o funcionamento do WeFIND perante os quarenta e seis requisitos funcionais do projeto. O Capítulo 9 finaliza a monografia com as considerações finais, a síntese dos resultados alcançados e as sugestões para a evolução futura da plataforma.

\else

A execução dos 25 casos de teste atestou a conformidade integral das regras de negócio implementadas para os 46 requisitos funcionais do WeFIND. As rotinas de validação no cliente impediram submissões com campos em branco ou formatos de dados inconsistentes, as operações no banco relacional preservaram as restrições de integridade referencial, o isolamento por políticas RLS assegurou o acesso estritamente autorizado aos registros e os canais em tempo real sustentaram a comunicação imediata entre os membros da comunidade.
