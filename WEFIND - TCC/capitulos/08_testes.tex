...
O planejamento metodológico prevê a aplicação individual do protocolo com uma amostra de 20 voluntários (tutores e colaboradores independentes) operando dispositivos físicos Android e iOS. Antes de responderem ao instrumento psicométrico, os participantes executarão um roteiro estruturado composto por cinco tarefas operacionais representativas do fluxo essencial da aplicação:
\begin{enumerate}
    \item \textbf{Criação de Conta e Autenticação:} realização de cadastro no sistema com credenciais seguras e login na plataforma;
    \item \textbf{Exploração do Mapa Interativo:} aplicação de filtros multicritério por espécie e consulta ao cartão rápido de localização de um animal;
    \item \textbf{Publicação de Animal Encontrado:} cadastro de publicação com foto da galeria e marcação manual das coordenadas do ponto de encontro no mapa;
    \item \textbf{Geração e Compartilhamento de Cartaz com Código QR:} renderização sob demanda da peça gráfica de busca para divulgação digital;
    \item \textbf{Gestão Preventiva e Carteirinha Digital (RG Pet):} consulta ao perfil do animal tutelado e visualização da carteirinha digital para coleiras.
\end{enumerate}
