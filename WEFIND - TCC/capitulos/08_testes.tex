...
\ifversaoorientadora
Para conferir rigor acadêmico e assegurar a cobertura integral do escopo do projeto, cada caso de teste foi desenhado e especificado para validar um conjunto articulado de Requisitos Funcionais (RF) estabelecidos no Capítulo 4. A suíte contempla 24 casos de teste estruturados (CT01 a CT24), combinando fluxos de caminho feliz (execução regular das rotinas), validações de robustez frente a dados inconsistentes e operações críticas em banco de dados e sensores nativos. Dessa forma, estrutura-se a cobertura planejada de 100\% dos 45 requisitos funcionais do aplicativo, cuja rastreabilidade individual completa frente às necessidades diagnosticadas na pesquisa de campo encontra-se catalogada no Apêndice D.
...
\else
Para conferir rigor acadêmico e assegurar a cobertura integral do escopo do projeto, cada caso de teste foi desenhado para validar um conjunto articulado de Requisitos Funcionais (RF) estabelecidos no Capítulo 4. A suíte contempla 24 casos de teste estruturados (CT01 a CT24), combinando fluxos de caminho feliz (execução regular das rotinas), validações de robustez frente a dados inconsistentes e operações críticas em banco de dados e sensores nativos. Dessa forma, atesta-se a cobertura de 100\% dos 45 requisitos funcionais do aplicativo, cuja rastreabilidade individual completa frente às necessidades diagnosticadas na pesquisa de campo encontra-se catalogada no Apêndice D.

O Quadro~\ref{quadro:testes} consolida a matriz integral dos 24 casos de testes funcionais executados sobre a versão estável da aplicação.
\fi
...
A integração equilibrada desses três eixos (os vinte e quatro casos de teste funcionais, o planejamento de usabilidade com a escala SUS e os ensaios de desempenho em campo) assegura uma base sólida para certificar o funcionamento do WeFIND perante os quarenta e cinco requisitos funcionais do projeto. O Capítulo 9 finaliza a monografia com as considerações finais, a síntese dos resultados alcançados e as sugestões para a evolução futura da plataforma.

\else

A execução dos 24 casos de teste atestou a conformidade integral das regras de negócio implementadas para os 45 requisitos funcionais do WeFIND. As rotinas de validação no cliente impediram submissões com campos em branco ou formatos de dados inconsistentes, as operações no banco relacional preservaram as restrições de integridade referencial, o isolamento por políticas RLS assegurou o acesso estritamente autorizado aos registros e os canais em tempo real sustentaram a comunicação imediata entre os membros da comunidade.
