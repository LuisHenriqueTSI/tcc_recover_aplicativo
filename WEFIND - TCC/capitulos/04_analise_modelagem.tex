...
Para a busca de animais, o aplicativo oferece os casos de uso \textit{Visualizar Publicações}, em formato de lista no feed, e \textit{Visualizar Animais no Mapa}, que mostra os animais diretamente sobre o mapa da região. Na tela do mapa, o usuário pode utilizar as extensões \textit{Ajustar Raio de Proximidade}, para definir a distância máxima da busca, e \textit{Atualizar Localização}, para atualizar sua posição via GPS.
...
O caso de uso \textit{Manter Publicação} concentra os requisitos de publicação de anúncios (RF09, RF15, RF16 e RF17), funcionando em conjunto com \textit{Selecionar Localização no Mapa} (RF10) e \textit{Informar Tutor Terceiro} (RF11). A visualização das postagens é atendida por \textit{Visualizar Publicações} (RF12 e RF13), \textit{Visualizar Detalhes da Publicação} (RF14) e \textit{Compartilhar Publicação} (RF34, RF35 e RF36).
