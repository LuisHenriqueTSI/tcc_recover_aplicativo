...
Este guia foi elaborado para orientar tutores, colaboradores voluntários e a sociedade na utilização prática do ecossistema móvel WeFIND. O manual sintetiza a jornada operacional do usuário por meio de roteiros visuais comentados, demonstrando o funcionamento de cada interface concebida para mitigar o desaparecimento e acelerar o reencontro comunitário de animais de estimação.

A estrutura do manual está organizada em seis passos sequenciais representativos das principais jornadas do sistema:
\begin{enumerate}[label=\textbf{Passo \arabic*:}, leftmargin=2.0cm]
    \item \textbf{Autenticação e Criação de Conta:} procedimentos de cadastro e acesso seguro;
    \item \textbf{Navegação no Mapa Interativo e Rotas:} exploração por GPS e cálculo viário;
    \item \textbf{Registro de Publicação de Animal Encontrado:} cadastro morfológico e georreferenciamento;
    \item \textbf{Detalhes e Geração de Cartazes Digitais:} ficha completa e exportação com código QR;
    \item \textbf{Comunicação Direta via Bate-Papo Privativo:} mensageria em tempo real com sigilo telefônico;
    \item \textbf{Cadastro Preventivo e Carteirinha Digital (RG Pet):} identificação preventiva e alerta de fuga.
\end{enumerate}

As seções a seguir detalham cada etapa por meio de capturas de tela reais do aplicativo, acompanhadas de instruções operacionais de uso.
...
\noindent
Ao iniciar o aplicativo WeFIND pela primeira vez, o cidadão é recepcionado pela tela de login, onde pode autenticar-se caso já possua conta ativa ou optar por ingressar na comunidade. Para novos participantes, o acesso inicia-se tocando no botão em destaque \textbf{``Cadastre-se''}, que conduz ao formulário unificado de criação de conta. A Figura~C1 ilustra as interfaces de autenticação e registro.
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \par\vspace{0.12cm}
        {\small (a) À esquerda: Formulário de Login}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        ...
        \par\vspace{0.12cm}
        {\small (b) À direita: Criação de Nova Conta}
    \end{minipage}
    \par\vspace{0.1cm}
    \fonte{Autoria própria (2026)}
\end{figure}

Conforme apresentado na Figura~C1, à esquerda (a), a tela de login solicita o e-mail cadastrado e a senha de acesso, contando com botão de alternância de visibilidade da senha e o atalho \textbf{``Esqueceu a senha?''} para recuperação via código OTP no WhatsApp. À direita (b), o formulário de cadastro requer o preenchimento de nome completo, endereço de e-mail, telefone celular para contato comunitário e senha de acesso segura. Após a validação sintática das entradas e o registro criptografado no serviço Supabase Auth, a sessão é automaticamente persistida no dispositivo e o usuário é redirecionado ao feed principal da aplicação.
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \par\vspace{0.12cm}
        {\small (a) À esquerda: Mapa com Marcadores}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        ...
        \par\vspace{0.12cm}
        {\small (b) À direita: Traçado de Rota ao Vivo}
    \end{minipage}
    \par\vspace{0.1cm}
    \fonte{Autoria própria (2026)}
\end{figure}

A visualização da Figura~C2 evidencia a convergência entre geolocalização e mobilidade urbana. À esquerda (a), o mapa renderiza marcadores georreferenciados diferenciados por cores e ícones conforme a espécie (cães, gatos, aves e outros animais domésticos), circundados por um raio de busca semitransparente ajustável pelo usuário. Ao tocar sobre qualquer marcador, o sistema exibe um cartão rápido com foto, nome e distância em quilômetros. Ao acionar o botão de navegação, a tela à direita (b) traça automaticamente a rota viária otimizada até as coordenadas do animal, informando o tempo estimado de deslocamento e a distância a percorrer para apoiar a ação imediata do voluntário ou tutor.

\clearpage

\subsection*{Passo 3: Registro de Publicação de Animal Encontrado na Rua}
\noindent
Ao avistar ou resgatar um animal desorientado em vias públicas, o cidadão inicia o fluxo de cadastro comunitário acionando o botão central \textbf{``+''} na barra inferior de navegação. O assistente orienta o preenchimento por etapas sequenciais, associando dados morfológicos, mídias fotográficas e coordenadas geográficas exatas. A Figura~C3 ilustra a etapa de caracterização do animal e o georreferenciamento no mapa interativo.
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \par\vspace{0.12cm}
        {\small (a) À esquerda: Morfologia e Fotos}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        ...
        \par\vspace{0.12cm}
        {\small (b) À direita: Fixação no Mapa}
    \end{minipage}
    \par\vspace{0.1cm}
    \fonte{Autoria própria (2026)}
\end{figure}

O procedimento de registro documentado na Figura~C3 garante a precisão das informações disponibilizadas à comunidade. À esquerda (a), o usuário define a categoria da publicação (animal encontrado na rua), anexa fotografias em alta definição capturadas na hora pela câmera ou selecionadas da galeria, e seleciona espécie, sexo, porte e cores predominantes da pelagem. Na etapa seguinte, exibida à direita (b), o sistema captura a posição atual do aparelho via GPS ou permite ao colaborador ajustar manualmente o pino sobre o logradouro exato onde o animal foi visto, acionando rotinas de geocodificação reversa para preencher automaticamente o endereço antes da publicação no feed geral.

\clearpage

\subsection*{Passo 4: Consulta de Detalhes e Geração de Cartazes com Código QR}
\noindent
Para viabilizar a identificação inequívoca de um animal anunciado, o sistema disponibiliza uma tela completa de detalhes com fotografias em alta resolução, dados biométricos e ferramentas de mobilização externa. A partir dessa tela, o tutor ou voluntário pode acionar a geração instantânea de cartazes digitais de busca. A Figura~C4 apresenta a visualização detalhada da publicação e a peça gráfica de divulgação gerada automaticamente.
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \par\vspace{0.12cm}
        {\small (a) À esquerda: Ficha da Publicação}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        ...
        \par\vspace{0.12cm}
        {\small (b) À direita: Cartaz com QR Code}
    \end{minipage}
    \par\vspace{0.1cm}
    \fonte{Autoria própria (2026)}
\end{figure}

A articulação entre a ficha descritiva e a divulgação externa visualiza-se na Figura~C4. À esquerda (a), a tela de detalhes consolida a galeria fotográfica com paginação deslizante, a descrição pormenorizada de marcas singulares e cicatrizes, o mapa de localização aproximada e os botões de ação comunitária (\textbf{``Vi o Pet''}, \textbf{``Entrar em Contato''} e \textbf{``Compartilhar Cartaz''}). Ao tocar na opção de compartilhamento, o componente nativo renderiza a tela à direita (b), diagramando automaticamente o cartaz de busca com fotografia destacada, dados essenciais, telefone protegido e código QR inteligente para escaneamento, com suporte de exportação nos formatos A4 para impressão e 9:16 ou 1:1 para redes sociais.

\clearpage

\subsection*{Passo 5: Comunicação Direta e Segura via Chat Interno}
\noindent
A articulação do resgate e a devolução segura do animal exigem um canal de comunicação direto, ágil e que preserve a privacidade dos membros da comunidade contra fraudes, assédios ou trotes. Para atender a essa exigência, o WeFIND disponibiliza um módulo de mensageria privativa em tempo real sustentado por WebSockets. A Figura~C5 ilustra a central de mensagens ativas e a conversa privativa em tempo real.
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \par\vspace{0.12cm}
        {\small (a) À esquerda: Caixa de Mensagens}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        ...
        \par\vspace{0.12cm}
        {\small (b) À direita: Chat em Tempo Real}
    \end{minipage}
    \par\vspace{0.1cm}
    \fonte{Autoria própria (2026)}
\end{figure}

Conforme ilustrado na Figura~C5, o sistema de mensageria promove o alinhamento seguro entre as partes envolvidas no resgate. À esquerda (a), a central de mensagens reúne as conversas ativas organizadas por animal, exibindo a fotografia do interlocutor, o status do anúncio, o carimbo de data e hora e o indicador de mensagens não lidas. Ao acessar uma conversa específica, a tela à direita (b) exibe o bate-papo criptografado e bidirecional com entrega imediata, no qual os membros podem trocar orientações de ponto de encontro e anexar fotografias comprobatórias de marcas singulares, eliminando por completo a necessidade de expor telefones particulares em murais públicos.
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \par\vspace{0.12cm}
        {\small (a) À esquerda: Painel Meus Pets}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        ...
        \par\vspace{0.12cm}
        {\small (b) À direita: Carteirinha (RG Pet)}
    \end{minipage}
    \par\vspace{0.1cm}
    \fonte{Autoria própria (2026)}
\end{figure}

A estrutura preventiva sintetizada na Figura~C6 consolida uma barreira de segurança de baixo custo contra o desaparecimento definitivo de animais. À esquerda (a), o painel de gestão preventiva lista os pets sob responsabilidade do usuário, exibindo foto, raça, idade e o botão de emergência \textbf{``Alerta de Fuga''}, que converte instantaneamente o prontuário em anúncio de busca ativo no mapa e feed com um único toque. Ao selecionar o animal, a tela à direita (b) exibe a carteirinha digital de identificação (RG Pet), contendo dados biométricos, contato de emergência e código QR exclusivo para impressão ou acoplamento a plaquinhas de coleira, permitindo que qualquer pessoa que encontre o animal na rua acesse a ficha pública segura escaneando a câmera do celular.
...
Consulta completa das características físicas e marcas singulares essenciais para a identificação do animal resgatado. & \textbf{RF14} & Tela de detalhes contendo carrossel de fotos em alta resolução, dados biométricos, mini-mapa com localização aproximada e atalhos de ação. & CT11 \\ \hline
...
Localização imediata de animais em bairros específicos ou por palavras-chave (nome do pet, raça). & \textbf{RF19} & Barra de pesquisa superior integrada ao mapa que filtra marcadores em tempo real por nome, raça ou logradouro, reposicionando a câmera do mapa. & CT10 \\ \hline
...
Alternativa de identificação visual imediata de baixo custo acessível a qualquer cidadão com smartphone sem exigir leitor de microchip. & \textbf{RF30} & Carteirinha digital preventiva com código QR unívoco gerado dinamicamente para fixação em plaquinhas de coleira, abrindo ficha pública segura. & CT18 \\ \hline
Atualização de fotografias recentes, histórico de vacinas ou novas características do animal em crescimento. & \textbf{RF31} & Tela de manutenção preventiva permitindo atualizar todos os atributos biométricos e sanitários do pet mantido pelo usuário. & CT17 \\ \hline
...
Continuidade no alinhamento das buscas e resgates entre os membros envolvidos. & \textbf{RF38} & Caixa de entrada exibindo conversas ativas agrupadas por animal, indicando mensagens não lidas, carimbo de horário e foto do interlocutor. & CT20 \\ \hline
Verificação visual segura de marcas e cicatrizes singulares para atestar a legitimidade da tutela antes do deslocamento presencial. & \textbf{RF39} & Botão de anexo na tela de conversa acoplando câmera e galeria com upload fotográfico imediato no bucket seguro de mensagens. & CT20 \\ \hline
