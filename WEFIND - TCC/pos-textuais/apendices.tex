...
\ifversaoorientadora
Este apêndice formaliza a cadeia integral de rastreabilidade bidirecional do projeto WeFIND, articulando as necessidades empíricas e oportunidades da análise comparativa diagnosticadas no Capítulo~3 com os 45 requisitos funcionais elicitados (Capítulo~4), as funcionalidades operacionais ativas desenvolvidas no aplicativo móvel (Capítulo~7) e os procedimentos experimentais de validação planejados (Capítulo~8).

A Tabela~D1 consolida a matriz segundo a estrutura de quatro eixos: \textbf{Necessidade Identificada} $\rightarrow$ \textbf{Requisito Funcional} $\rightarrow$ \textbf{Funcionalidade Implementada} $\rightarrow$ \textbf{Caso de Teste Associado}, demonstrando que 100\% dos 45 requisitos funcionais contam com funcionalidades ativas implementadas no código-fonte e casos de teste formalmente planejados para a etapa de homologação prática.
\else
Este apêndice formaliza a cadeia integral de rastreabilidade bidirecional do projeto WeFIND, articulando as necessidades empíricas e oportunidades da análise comparativa diagnosticadas no Capítulo~3 com os 45 requisitos funcionais elicitados (Capítulo~4), as funcionalidades operacionais ativas desenvolvidas no aplicativo móvel (Capítulo~7) e os procedimentos experimentais de validação executados (Capítulo~8).

A Tabela~D1 consolida a matriz segundo a estrutura de quatro eixos: \textbf{Necessidade Identificada} $\rightarrow$ \textbf{Requisito Funcional} $\rightarrow$ \textbf{Funcionalidade Implementada} $\rightarrow$ \textbf{Caso de Teste Associado}, atestando que 100\% dos 45 requisitos funcionais foram efetivamente implementados no código-fonte e homologados com aprovação integral em testes com dispositivos físicos.
\fi
